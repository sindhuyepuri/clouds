...
export class Terrain implements MaterialObject {
  ...
  constructor(light: Vec4) {
    ...
    for (let i = -width/2; i < width/2; i+=incr) {
      for (let j = 0; j < depth; j+=incr) { // was 300
        ...
        shadows[i + width/2][j] = this.inShadow(new Vec3(v1.xyz));
    ...
    for (let i = -width/2; i < width/2; i += 1) {
      for (let j = 0; j < depth; j += 1) {
        this.shadow.push(shadows[i + width/2][j]);
        this.shadow.push(shadows[i + width/2][j + 1]);
        this.shadow.push(shadows[i + 1 + width/2][j + 1]);
        this.shadow.push(shadows[i + 1 + width/2][j]);
      }
    }
